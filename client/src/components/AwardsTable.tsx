...
function AnimatedRow({ award, onHover, onLeave }: AnimatedRowProps) {
  ...
  return (
    ...
    >
      <td className="py-8 pr-8 text-fuchsia-400 font-semibold text-lg">
        {award.number}
      </td>
      <td className="py-8 pr-8 text-white font-medium text-lg">
        {award.award}
      </td>
      <td className="py-8 pr-8 text-gray-400">{award.project}</td>
      <td className="py-8 text-right text-fuchsia-400 font-medium">
        {award.year}
      </td>
    </motion.tr>
  );
...
function AnimatedHeader() {
  ...
  return (
    ...
    >
      <th className="text-left text-gray-500 font-medium py-6 pr-8 text-sm uppercase tracking-wider">
        #
      </th>
      <th className="text-left text-gray-500 font-medium py-6 pr-8 text-sm uppercase tracking-wider">
        project
      ...
      <th className="text-right text-gray-500 font-medium py-6 text-sm uppercase tracking-wider">
        Year
      </th>
    </motion.tr>
  );
...
export default function AwardsTable() {
  ...
  const awards: Award[] = [
    { number: "01", award: "Hoarding Wizard", project: "AdTech", year: "2024", url: "https://hoardingwizard.com" },
    {
      number: "02",
      award: "Elyx",
    ...
    { number: "03", award: "City Care", project: "Health Care", year: "2025", url: "https://play.google.com/store" },
    { number: "04", award: "True levy", project: "Fintech", year: "2025", url: "https://truelevy.in/" },
  ];
  ...
  return (
    <section 
      ref={sectionRef}
      className="py-24 lg:py-32 bg-black relative" 
      style={{ cursor: hoveredRowUrl ? "none" : "auto" }}
      data-testid="section-awards"
    >
      {/* Custom cursor circle - fixed position for accurate tracking */}
      <motion.div
        className="pointer-events-none fixed z-[9999] w-20 h-20 rounded-full flex items-center justify-center text-white font-medium text-sm"
        animate={{
